...
def plot_bottom_left():
    sales_data = duckdb.sql(
        ...
        WITH sales_data AS (
            SELECT 
            Scenario, Business_Unit, {','.join(all_months)} 
            FROM df 
            WHERE Year='2023' 
            AND Account='Sales'
            AND Business_Unit IN ('Software', 'Hardware', 'Advertising')
        )
    ...
    # Convert 'Business_Unit' column to lowercase
    sales_data['business_unit'] = sales_data['business_unit'].str.lower()
    sales_data['sales'] = sales_data['sales'].round()

    fig = px.line(
        sales_data,
        x="month",
        y="sales",
        color="business_unit",  # Color by business unit
        line_dash="Scenario",   # Use line dash to distinguish scenarios
        markers=True,
        text="sales",
        title="Monthly Sales vs Monthly Budget (2023)",
    )
